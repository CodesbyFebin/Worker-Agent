...
router.get("/workflows", requirePermission("workflow:read"), async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.post("/goals", requirePermission("agent:dispatch"), async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.get("/campaigns", requirePermission("campaign:read"), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { orgId } = requireOrg(req);
...
router.get("/youtube/channels", requirePermission("youtube:read"), async (req, res, next) => {
  try {
    const { orgId } = requireOrg(req);
...
router.get("/knowledge/search", requirePermission("knowledge:read"), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { orgId } = requireOrg(req);
...
router.get("/knowledge/semantic", requirePermission("knowledge:read"), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { orgId } = requireOrg(req);
    const query = typeof req.query.query === "string" ? req.query.query : undefined;
    if (!query || query.length < 1 || query.length > 1000) {
...
router.post("/knowledge/embeddings", requirePermission("knowledge:write"), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { orgId } = requireOrg(req);
...
export function buildOpenApiDocument(): Record<string, unknown> {
  return {
    ...
    paths: {
      ...
      "/api/v1/workflows": { get: { summary: "List workflows", responses: { 200: { description: "OK" } } } },
      "/api/v1/goals": { post: { summary: "Dispatch goal", responses: { 201: { description: "Created" } } } },
      "/api/v1/campaigns": { get: { summary: "List campaigns", responses: { 200: { description: "OK" } } } },
      "/api/v1/youtube/channels": { get: { summary: "List YouTube channels", responses: { 200: { description: "OK" } } } },
      "/api/v1/knowledge/search": { get: { summary: "Search knowledge", responses: { 200: { description: "OK" } } } },
      "/api/v1/knowledge/semantic": { get: { summary: "Semantic search", responses: { 200: { description: "OK" } } } },
      "/api/v1/knowledge/embeddings": { post: { summary: "Upsert embedding", responses: { 201: { description: "Created" } } } },
    },
  };
}
